...
export function IdentitySection({ fieldErrors, onClearFieldError }: IdentitySectionProps) {
  return (
    <section className="paper-panel rounded-[34px] px-6 py-7 sm:px-8 sm:py-8">
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2">
        ...
        <label className="space-y-2">
          ...
          <input
            ...
            placeholder="Mother"
            onChange={() => onClearFieldError("relationship")}
          />
        ...
        <label className="space-y-2">
          ...
          <input name="avatar" type="file" accept="image/*" className="input-quiet w-full" />
        </label>
      </div>
      ...
      <label className="mt-5 block space-y-2">
        ...
        <textarea
          ...
          onChange={() => onClearFieldError("description")}
        />
...
type VoiceSectionProps = {
  ...
  setStarterVoiceId: Dispatch<SetStateAction<string>>;
  onRecorded: (file: File) => void;
  onRemoveRecordedSample: (sampleId: string) => void;
  onRecorderError: (message: string | null) => void;
};
    ...
    <section className="paper-panel rounded-[34px] px-6 py-7 sm:px-8 sm:py-8">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div className="rounded-[26px] border border-[rgba(103,112,100,0.12)] bg-[rgba(223,228,218,0.64)] px-4 py-3 text-sm leading-6 text-[var(--sage-deep)]">
          {recordedSamples.length > 0
            ? `${recordedSamples.length} recorded sample${recordedSamples.length === 1 ? "" : "s"} saved`
            : "No recordings yet"}
        </div>
      </div>
      ...
      <div className="mt-6 rounded-[28px] border border-[var(--line)] bg-[rgba(255,255,255,0.72)] px-5 py-5">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <VoiceRecorder
            ...
            onError={(message) => onRecorderError(message)}
            onRecorded={(file) => {
        ...
        {recordedSamples.length > 0 ? (
          <div className="mt-4 space-y-3">
            {recordedSamples.map((sample, index) => (
              ...
              >
                <div>
                  <p className="text-sm font-medium text-[var(--sage-deep)]">Recording {index + 1}</p>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button
                    type="button"
                    onClick={() => onRemoveRecordedSample(sample.id)}
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-2">
        <label className="space-y-2">
          ...
          <input name="voiceSamples" type="file" accept="audio/*,video/*" multiple className="input-quiet w-full" />
        </label>
        <label className="space-y-2">
          <span className="text-sm font-medium text-[var(--sage-deep)]">Upload chat screenshots</span>
          <input name="screenshots" type="file" accept="image/*" multiple className="input-quiet w-full" />
        </label>
      </div>
...
type MemorySectionProps = {
  presencePreset: "soft" | "steady" | "close";
  setPresencePreset: Dispatch<SetStateAction<"soft" | "steady" | "close">>;
  modePreset: "mixed" | "text" | "voice_note";
  setModePreset: Dispatch<SetStateAction<"mixed" | "text" | "voice_note">>;
};
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2">
        <div className="rounded-[28px] border border-[var(--line)] bg-[rgba(255,255,255,0.72)] px-5 py-5">
          <p className="text-sm font-medium text-[var(--sage-deep)]">
            How much of them should be in the room
          </p>
...
export function SafetySection({ createError, isSubmitting }: SafetySectionProps) {
  return (
    <section className="paper-panel rounded-[34px] px-6 py-7 sm:px-8 sm:py-8">
      <div className="grid gap-6 md:grid-cols-[1fr_auto] md:items-end">
        ...
        <div className="rounded-[28px] bg-[rgba(63,74,66,0.96)] px-5 py-5 text-white md:max-w-sm">
          ...
          {createError ? <p className="mt-3 text-sm text-[rgba(190,80,70,0.9)]">{createError}</p> : null}
          <button
            type="submit"
            disabled={isSubmitting}
